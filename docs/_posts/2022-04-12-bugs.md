...
list is not actually decremented when monsters are defeated.
...
The game is supposed to record the creature total when you defeat a fixed
encounter or flee from one. This would allow you to defeat the Fire Trolls a few
at a time, reviving slain heroes between attempts before taking on the Dark Lord
solo. This would make the final battle easier.

The bug occurs because, for some reason, the end-of-battle summary screen calls
the party move function twice per hero, pushing you 12 squares in the direction
you last moved. Since you're no longer on a fixed encounter square, the code to
save fixed encounter square never triggers.

_Solution:_ Unpack the main game executable `AVALON_E` using
[xfddecrunch](https://aminet.net/package/util/pack/xfdmaster).
Using a hex editor
(e.g. [FileX](https://aminet.net/package/disk/moni/FileX-68K),
search for the bytes `5c89 6100 4a1c 4250`. Overwrite `6100 4a1c` with
`4e71 4e71`. The game should now detect correctly.

_Alternate solution:_ It's possible, using save state editing, to force the win
condition to trigger immediately, allowing you to see the victory screen and
credits roll as intended.
...
animations continue to play, but the controls become unresponsive. The bug may
also reportedly crash the game instead.
...
### Spinners freeze the game

_Affects: One version of Dungeons of Avalon II_

A bug has been reported
([issue #7](https://github.com/tetracorp/dungeons-of-avalon/issues/7))
affecting one version of DoA2. Looking at a spinner can cause the game to
freeze, similarly to the aforementioned final level encounter freeze bug.

The source of of the affected version is not known. Reportedly, it featured the
level 16 characters and final level freeze bug fix from the Sonderausgabe 1/95
re-release version, but did not come from that particular CD-ROM.

_Solution:_ Enter spinners by sidestepping. Alternatively, play the
Sonderausgabe 1/95 re-release.
...
_Affects: Both games, all versions_

Items with limited charges can be recharged by selling them to the shop, then
buying them back.
...
_Affects: Dungeons of Avalon I, official English version_

[Dungeon level 5](../maps/doa1-dungeon5.html) is missing one of the floor
messages in the file `S/TX04`. Of the 18 messages, the third-last is missing a
translation, so the two following.

- "NO KEY? USE THE SWITCH ON THE WALL!" should read "ARE YOU ALL HERE??"
- The square to the east of it should read "NO KEY? USE THE SWITCH ON THE WALL!"
- "ARE YOU ALL HERE??" should add a new line meaning "THEN I WILL NOW RAISE
  STRONGER GUNS AGAINST YOU!"
